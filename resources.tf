...
resource "docker-image" "bgg-backend" {
  name = "chukmunlee/bgg-backend:${var.backend_version}"
}

resource "docker-network" "bgg-network" {
  name = "${var.app_namespace}-bgg-net"
}

resource "docker-volume" "data-vol" {
  name = "${var.app_namespace}-data-vol"
}

## Spin off a container
resource "docker-container" "bgg-database" {
...
resource "docker-container" "bgg-backend" {
...
resource "local_file" "root_at_nginx" {
    filename = "roots@${digitalocean_droplet.nginx.ipv4_address}"
    content = ""
